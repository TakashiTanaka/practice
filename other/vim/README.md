...
|--------|----|
|h / j / k / l|左/下/上/右|
|gj / gk |表示上の一行上へ移動/表示上の一行下を移動（ただこれは面倒なのでvimrcでnoremapした方が良いかも）|
|Control + u|半画面分上へ（多分upの意味）|
|Control + d|半画面分下へ（多分downの意味）|
|a|カーソルがある行の末尾にカーソルが移動し、insertモードになる（vscode-neovimだとカーソルが当たっている箇所のすぐ後ろにinsertモードになる。行の末尾に移動してinsertにしたいなら、Aでいけた）|
|^|行頭へ移動|
|$|行末へ移動(Shift押しながら)|
...
|o|カーソルのある行の下に行を追加、insertへ|
|O|カーソルのある行の上に行を追加、insertへ|
|Control + a|数値を選択している状態で押すとインクリメント|
|Control + x|数値を選択している状態で押すとデクリメント|
...
[超融合\!時空を越えた絆 Neo Vim\(VSCode\)を試してみた](https://zenn.dev/yutakatay/articles/vscode-neovim)